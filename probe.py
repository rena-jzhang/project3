...
ROOT = '/home/ubuntu/efs/X-FACTR/'
SUB_LABEL = '##'
PREFIX_DATA = '../LAMA/'
VOCAB_PATH = PREFIX_DATA + 'pre-trained_language_models/common_vocab_cased.txt'
RELATION_PATH = ROOT + 'data/TREx-relations.jsonl'
PROMPT_LANG_PATH = ROOT +'data/TREx_prompts.csv'
LM_NAME = {
    # multilingual model
...
DATASET = {
    ...
    'mlamaf': {
        'entity_path':ROOT + 'data/mTRExf/sub/{}.jsonl',
        'entity_lang_path': ROOT +'data/mTRExf_unicode_escape.txt',
        'entity_gender_path': ROOT +'data/mTRExf_gender.txt',
        'entity_instance_path': ROOT +'data/mTRExf_instanceof.txt',
        'alias_root':  ROOT + 'data/alias/mTRExf',
        'multi_rel':  ROOT + 'data/mTRExf_multi_rel.txt',
        'is_cate': ROOT + 'data/mTRExf_is_cate.txt',
    }
}
...
class ProbeIterator(object):
    def __init__(self, args: argparse.Namespace, tokenizer):
        ...
        if args.use_hint:
            self.hint_prompt_model = Prompt.from_lang(
                 args.hint_lang, self.entity2gender, self.entity2instance,
                args.disable_inflection, args.disable_article)
    ...
    def get_hint_queries(self, fact_path: str) -> Tuple[List[Dict], List[Union[int, float]]]:
        HINT_LANG = self.args.hint_lang
        LANG = self.args.lang

        queries: List[Dict] = []
        num_skip = not_exist = num_multi_word = num_single_word = 0
        hint_queries: List[Dict] = []
        with open(fact_path) as fin:
            for l in fin:
                l = json.loads(l)
                # hint_exist = False
                sub_exist = HINT_LANG in self.entity2lang[l['sub_uri']]
                obj_exist = HINT_LANG in self.entity2lang[l['obj_uri']]
                # sub_exist = LANG in self.entity2lang[l['sub_uri']]
                # obj_exist = LANG in self.entity2lang[l['obj_uri']]
                # hint_exist = hint_sub_exist and hint_obj_exist
                if self.restricted_facts is not None and \
                        (l['sub_uri'], l['obj_uri']) not in self.restricted_facts:
                    continue
                ...
                # resort to English label
                if self.args.sub_obj_same_lang:
                    l['sub_label'] = self.entity2lang[l['sub_uri']][HINT_LANG if exist else 'en']
                    l['obj_label'] = self.entity2lang[l['obj_uri']][HINT_LANG if exist else 'en']
                else:
                    l['sub_label'] = self.entity2lang[l['sub_uri']][HINT_LANG if sub_exist else 'en']
                    l['obj_label'] = self.entity2lang[l['obj_uri']][HINT_LANG if obj_exist else 'en']
                sub_label_t = tokenizer_wrap(self.tokenizer, HINT_LANG, False, l['sub_label'])
                obj_label_t = tokenizer_wrap(self.tokenizer, HINT_LANG, False, l['obj_label'])
                if self.unk in sub_label_t or self.unk in obj_label_t:
                    not_exist += 1
                    continue
                if len(obj_label_t) <= 1:
    ...
    def hint_batcher(self, queries: List[Dict], hint_queries, prompt: str, hint_prompt: str) -> Tuple[List, Tuple, Tuple]:
        LANG = self.args.lang
        HINT_LANG = self.args.hint_lang
        NUM_MASK = self.args.num_mask
        ...
        for b in tqdm(range(0, len(queries), self.args.batch_size), disable=True):
            query_batch = queries[b:b + self.args.batch_size]
            # hint_batch = hint_queries[b:b + self.args.batch_size]
            ...
            for query in query_batch:
                ...
                instance_x, _ = self.prompt_model.fill_x(
                    prompt, query['sub_uri'], query['sub_label'])
                hint_sub_uri = query['sub_uri']
                instance_hint = hint_prompt
                if HINT_LANG in self.entity2lang[hint_sub_uri]:
                    hinting = True
                    hint_sub = self.entity2lang[hint_sub_uri][HINT_LANG]
                    instance_hint, _ = self.hint_prompt_model.fill_x(
                        hint_prompt, hint_sub_uri, hint_sub)
                hint_obj_uri = query['obj_uri']
                if hinting and HINT_LANG in self.entity2lang[hint_obj_uri]:
                    hint_obj = self.entity2lang[hint_obj_uri][HINT_LANG]
                    if hint_obj == query['obj_label']:
                        hinting = False
                    else:
                        instance_hint, _ = self.hint_prompt_model.fill_y(
                            instance_hint, hint_obj_uri, hint_obj,
                            num_mask=0, mask_sym=self.mask_label)
                else:
                    hinting = False
                
                # fill in objects
                instance_xys: List[str] = []
                # instance_hints: List[str] = []
                # instance_hints.append(instance_hint)
                if self.args.use_gold:
                    instance_xy, obj_label = self.prompt_model.fill_y(
                        instance_x, query['obj_uri'], query['obj_label'])
                    if self.args.dry_run and self.args.dry_run <= 50:
                    ...
                    for nm in range(NUM_MASK):
                        ...
                        instance_xys.append(instance_xy)
                if hinting:
                    hint_tensor = torch.tensor(tokenizer_wrap(self.tokenizer, HINT_LANG, True, instance_hint))
                # tokenize sentences
                for instance_xy in instance_xys:
                    if re.match('\[.*X.*\]', instance_xy) or re.match('\[.*Y.*\]', instance_xy):
                        raise Exception('inflection missing from "{}"'.format(instance_xy))
                    if not self.args.use_gold and instance_xy.find(self.mask_label) == -1:
                        raise Exception('not contain mask tokens "{}"'.format(instance_xy))
                    instance_tensor = torch.tensor(tokenizer_wrap(self.tokenizer, LANG, True, instance_xy))
                    if hinting:
                        inp_tensor.append(torch.cat((hint_tensor, instance_tensor[1:]), 0))
                    else:
                        inp_tensor.append(instance_tensor)
                # tokenize gold object
                obj = np.array(tokenizer_wrap(self.tokenizer, LANG, False, obj_label)).reshape(-1)
                obj_li.append(obj)
        ...
        for pattern, fact_path in self.relation_iter(pids=pids):
            ...
            try:
                ...
                with CsvLogFileContext(log_filename, headers=headers) as csv_file, \
                    ...
                    # if self.args.use_hint:
                    #     hint_queries, _ = self.get_hint_queries(fact_path)
                    # get prompt
                    if self.args.prompts:
                        ...
                        if self.args.use_hint:
                            hint_prompts =  [self.prompt_lang[self.prompt_lang['pid'] == relation][HINT_LANG].iloc[0]]
                        prompts = [self.prompt_lang[self.prompt_lang['pid'] == relation][LANG].iloc[0]]
                    if not self.args.use_hint:
                        hint_prompts = [None for _ in prompts]

                    correct_facts: Set[Tuple[str, str]] = set()

                    for prompt, hint_prompt in zip(prompts, hint_prompts):
                        acc, len_acc, acc_ori, len_acc_ori = [], [], [], []
                        if self.args.use_hint:
                            batch = self.hint_batcher(queries, None, prompt, hint_prompt)
                        else:
                            batch = self.batcher(queries, prompt)
                        for qbi, \
                            (query_batch,
                             (inp_tensor, attention_mask, mask_ind),
                             (obj_li, obj_ori_li)) in tqdm(enumerate(batch), disable=True):
                            ...
                            for i, avg_log in enumerate((logprob * mask_ind).sum(-1) / mask_len_norm):
                                ...
                                def get_all_pred():
                                    ...
                                    return results
                                test_sentence = merge_subwords(inp, tokenizer, merge=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hint_lang', type=str, help='choose hint language', choices=['en', 'fr', 'nl', 'es', 'zh'], default='en')
    ...
    parser.add_argument('--use_hint', action='store_true', help='use hints')
